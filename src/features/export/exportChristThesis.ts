...
\\newcommand{\\DepartmentVision}{${texEscape(m.departmentVision || '')}}
\\newcommand{\\DepartmentMission}{${texEscape(m.departmentMission || '')}}
\\newcommand{\\DepartmentPEOs}{${texEscape(m.departmentPEOs || '')}}
\\newcommand{\\DepartmentPSOs}{${texEscape(m.departmentPSOs || '')}}
\\newcommand{\\ReferenceMode}{normal}
`;
}

export function buildChristThesis(doc: TemplytXDocument, numberingStyle: NumberingStyle = 'continuous'): string {
...
\\pagebreak`;
  ...
  for (const b of doc.blocks) {
    ...
    if (b.type === 'section') {
      ...
      const star = (b as { unnumbered?: boolean }).unnumbered ? '*' : '';
      if (lvl <= 1) { parts.push(`\\chapter${star}{${title.toUpperCase()}}`); openedChapter = true; }
      else if (lvl === 2) { if (!openedChapter) { parts.push('\\chapter{INTRODUCTION}'); openedChapter = true; } parts.push(`\\section${star}{${title}}`); }
      else if (lvl === 3) { if (!openedChapter) { parts.push('\\chapter{INTRODUCTION}'); openedChapter = true; } parts.push(`\\subsection${star}{${title}}`); }
      else { if (!openedChapter) { parts.push('\\chapter{INTRODUCTION}'); openedChapter = true; } parts.push(`\\subsubsection${star}{${title}}`); }
      ...
      else parts.push(`\\begin{equation}\\label{${lbl}}\n${b.latex || ''}\n\\end{equation}`);
...
\\end{thebibliography}` : '';
  ...
  // Self-contained main file. Mirrors CHRIST's original ProjectReport.tex
  // preamble + full front/back-matter input sequence exactly, substituting the
  // generated ProjectVariables and chapter content. This guarantees the output
  // matches the official template (36-page structure).
  const chapters = body || '\\chapter{INTRODUCTION}\n';
